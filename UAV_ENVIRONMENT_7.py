                ...
                # Task-selection mode check: when drone has serving_order_id but no planned_stops
                    ...
                    if order['status'] == OrderStatus.ASSIGNED:
                        ...
                        if drone['status'] in [DroneStatus.RETURNING_TO_BASE]:
                            issues.append(f"[Legacy] 订单 {order_id} 已分配但无人机 {drone_id} 正在返航: {drone['status']}")
                        ...
                        if drone['status'] not in [DroneStatus.FLYING_TO_CUSTOMER, DroneStatus.DELIVERING, DroneStatus.FLYING_TO_MERCHANT]:
                            if not ('batch_orders' in drone and order_id in drone['batch_orders']):
                                issues.append(f"[Legacy] 订单 {order_id} 已取货但无人机 {drone_id} 状态不匹配: {drone['status']}")
        ...
        if self.debug_state_warnings:
            ...
            if self.time_system.current_step % 64 == 0:
                if consistency_issues:
                    ...
                    print(f"[Step {self.time_system.current_step}] 状态一致性问题计数: {len(consistency_issues)} ({category_str})")
                else:
